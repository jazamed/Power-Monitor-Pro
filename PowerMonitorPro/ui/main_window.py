...
from backends import FNIRSIBackend, SUPPORTED_PROFILES, AtorchT3Backend, RP2040INA260Backend, ESP32INA260Backend
...
class DiagnosticsWindow(QWidget):
    ...
    def update_info(self):
        ...
        lines = []
        lines.append("POWER MONITOR PRO - DIAGNOSTICS")
        lines.append("=" * 72)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("POWER MONITOR PRO")
        ...
        self.chk_live_log = QCheckBox("Live CSV")
    ...
    def apply_backend_connected_state(self):
        ...
        self.setWindowTitle(f"POWER MONITOR PRO - {device_name}")
    ...
    def on_live_csv_toggled(self, checked):
        ...
        if checked:
            ...
            if file_path:
                ...
                self.lbl_status.setText("LIVE CSV READY")
                self.lbl_status.setStyleSheet(f"color: {UIColors.STATUS_OK};")
            else:
    ...
    def toggle_logging(self):
        ...
        if not self.logger.enabled:
            ...
            if live_mode:
                file_path = self.live_csv_path
                if not file_path:
                    self.lbl_status.setText("NO LIVE CSV FILE")
                    self.lbl_status.setStyleSheet(f"color: {UIColors.STATUS_WARN};")
                    return
    ...
    def start_playback(self, file_path: str):
        ...
        self.lbl_status.setStyleSheet(f"color: {UIColors.STATUS_OK};")
        self.setWindowTitle("POWER MONITOR PRO - CSV Playback")
